...
void InnerWidget::updateSearchResult(not_null<PeerData*> peer) {
	if (_state == WidgetState::Filtered) {
		const auto id = ranges::find(
			_idSearchResults,
			peer,
			&PeerSearchResult::peer);
		if (id != end(_idSearchResults)) {
			const auto top = idSearchOffset();
			const auto index = (id - begin(_idSearchResults));
			rtlupdate(
				0,
...
void InnerWidget::preloadRowsData() {
	...
	if (_state == WidgetState::Default) {
		...
		from = (yFrom - idSearchOffset()) / st::dialogsRowHeight;
		if (from < 0) from = 0;
		if (from < _idSearchResults.size()) {
			const auto to = std::min(
				((yTo - idSearchOffset()) / st::dialogsRowHeight) + 1,
				int(_idSearchResults.size()));
			for (; from < to; ++from) {
				_idSearchResults[from]->peer->loadUserpic();
			}
		}
...
bool InnerWidget::selectChildByIndex(int index) {
	...
	if (_state == WidgetState::Default) {
		...
		_hashtagSelected = _filteredSelected = _idSearchSelected
			= _peerSearchSelected = _previewSelected = _searchedSelected = -1;
		switch (ref->cohort) {
	...
	if (_state == WidgetState::Default) {
		...
		case AccessibilityCohort::Filtered:
			return RowAccessibilityName(
				...
				_filterId);
		case AccessibilityCohort::IdSearch: {
			const auto &r = _idSearchResults[ref->local];
